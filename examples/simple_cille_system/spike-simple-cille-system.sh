...
# for the simple_cille_system environment using Spike.
#
...
usage () {
    echo >&2 "Usage:  spike-simple-cille-system [-h] [-v] [-n] [--] [options] <elf>"
    echo >&2
    echo >&2 "  Where <elf> is the path to an ELF binary that was"
    echo >&2 "  compiled for the simple_cille_system environment. Any arguments "
    echo >&2 "  in <options> will be passed to Spike after those to set up "
    echo >&2 "  the environment for simple_cille_system tests."
    echo >&2
...
# Here are the Spike options you need to run in a simple_cille_system world.
declare -a ss_opts
ss_opts=("--isa=rv32imc"
         "--log-commits"
         "-l"
