...
fusesoc --cores-root=. run --target=sim --setup --build \
        lowrisc:ibex:ibex_simple_mads_system $(util/ibex_config.py small fusesoc_opts)
```
...
Simple System related software can be found in `examples/sw/simple_mads_system`.

To build the hello world example, from the Ibex reposistory root run:

```
make -C examples/sw/simple_mads_system/hello_test
```

The compiled program is available at
`examples/sw/simple_mads_system/hello_test/hello_test.elf`. The same directory also
...
`examples/sw/simple_mads_system/common/common.mk` may need altering to point to the
...
```
./build/lowrisc_ibex_ibex_simple_mads_system_0/sim-verilator/Vibex_simple_mads_system [-t] --meminit=ram,<sw_elf_file>
```

`<sw_elf_file>` should be a path to an ELF file  (or alternatively a vmem file)
built as described above. Use
`./examples/sw/simple_mads_system/hello_test/hello_test.elf` to run the `hello_test`
binary.
...
`-DVM_TRACE_FMT_FST` in ibex_simple_mads_system.core before building the simulator
binary.
...
* `ibex_simple_mads_system.log` - The ASCII output written via the output peripheral
* `ibex_simple_mads_system_pcount.csv` - A CSV of the performance counters
...
```
fusesoc --cores-root=. run --target=sim --tool=vcs --setup --build lowrisc:ibex:ibex_simple_mads_system --RV32E=0 --RV32M=ibex_pkg::RV32MFast --SRAMInitFile=`<sw_vmem_file>`
```

`<sw_vmem_file>` should be a path to a vmem file built as described above, use
`./examples/sw/simple_mads_system/hello_test/hello_test.vmem` to run the `hello_test`
binary.

To run the simulator:

```
./build/lowrisc_ibex_ibex_simple_mads_system_0/sim-vcs/lowrisc_ibex_ibex_simple_mads_system_0
```
...
```
fusesoc --cores-root=. run --target=sim --tool=rivierapro lowrisc:ibex:ibex_simple_mads_system --RV32E=0 --RV32M=ibex_pkg::RV32MFast --SRAMInitFile=\"$(readlink -f <sw_vmem_file>)\"
```
