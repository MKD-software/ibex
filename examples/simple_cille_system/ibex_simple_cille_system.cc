...
#include "Vibex_simple_cille_system__Syms.h"
...
std::string SimpleSystem::GetIsaString() const {
  const Vibex_simple_cille_system &top = _top;
  assert(top.ibex_simple_cille_system);

  std::string base = top.ibex_simple_cille_system->RV32E ? "rv32e" : "rv32i";

  std::string extensions;
  if (top.ibex_simple_cille_system->RV32M)
    extensions += "m";

  extensions += "c";

  switch (top.ibex_simple_cille_system->RV32B) {
    case 0:  // RV32BNone
      break;
...
bool SimpleSystem::Finish() {
  ...
  svSetScope(svGetScopeFromName("TOP.ibex_simple_cille_system"));

  std::cout << "\nPerformance Counters" << std::endl
            << "====================" << std::endl;
  std::cout << ibex_pcount_string(false);

  std::ofstream pcount_csv("ibex_simple_cille_system_pcount.csv");
